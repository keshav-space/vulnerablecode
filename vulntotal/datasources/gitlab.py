...
from vulntotal.vulntotal_util import gitlab_constraints_satisfied
...
class GitlabDataSource(DataSource):
    ...
    def datasource_advisory(self, purl) -> Iterable[VendorData]:
        ...
        location = download_subtree(package_slug)
        if not location:
        ...
        if location:
            interesting_advisories = parse_nteresting_advisories(location, purl.version)
...
def get_gitlab_style_slug(ecosystem, package_slug):
    url = "https://advisories.gitlab.com/resources/cves.json.bin"
    response = requests.get(url)
    lz = lzstring.LZString()
    decompressed = lz.decompressFromBase64(response.text)
    for vul in json.loads(decompressed):
        if vul["card_component"].lower() == package_slug.lower():
            return vul["card_component"]
...
def download_subtree(package_slug: str):
...
def parse_nteresting_advisories(location, version) -> Iterable[VendorData]:
    path = Path(location)
    glob = "**/*.yml"
